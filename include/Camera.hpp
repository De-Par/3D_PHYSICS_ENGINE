...
namespace gra {
    ...
    public:
        ...
        Camera(glm::vec3 worldPosition, glm::vec3 viewDirection, glm::vec3 upDirection, unsigned int width,
               unsigned int height, float farplane, float nearplane, float fov) {
        ...
        void rotAroundCenter(float newMouseX, float newMouseY) {
            float normalMouseX = (newMouseX - (float)height / 2) / (float)height;
            float normalMouseY = (newMouseY - (float)width / 2) / (float)width;
            float rotX = (float)(normalMouseY - curScreenY) * rotationSensitivity;
            float rotY = (float)(normalMouseX - curScreenX) * rotationSensitivity;

            glm::vec3 rotCenter = viewDirection * distanceToRotationCenter + worldPosition;
            glm::vec3 rotCenterView = -viewDirection;

            // Calculates upcoming vertical change in the Orientation
            glm::vec3 newOrientation = glm::rotate(rotCenterView,
                                                   glm::radians(rotX),
                                                   glm::normalize(glm::cross(rotCenterView, upDirection)));
        ...
        void translatePos(float newMouseX, float newMouseY) {
            float normalMouseX = (newMouseX - (float)height / 2) / (float)height;
            float normalMouseY = (newMouseY - (float)width / 2) / (float)width;
            auto translateX = (float)(normalMouseX - curScreenX);
            auto translateY = (float)(normalMouseY - curScreenY);
        ...
        void setShaderValues(Shader* shader) const {
