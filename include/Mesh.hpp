...
namespace gra {
    ...
    public:
        ...
        Mesh(std::vector<Vertex>& verts, std::vector<uint>& indices) {
            ...
            std::cout << "num of trians: " << numOfTrians << "\n";
        ...
        void setColor(glm::vec4 col) {
            specularTexture = new Texture(glm::vec4(col.x, col.y, col.z, 1.0));
        }

        void loadTexture(const char* filepath) {
            specularTexture = new Texture(filepath);
        }
        ...
        void draw() {
            ...
            glDrawElements(GL_TRIANGLES, numOfTrians * 3, GL_UNSIGNED_INT, nullptr);
            glBindVertexArray(0);

            for (auto & debugMeshe : debugMeshes) {
                debugMeshe->draw();
            }
        }
        ...
        void addDebugMeshs(const std::shared_ptr<Mesh>& debugMesh) {
